...
// ====== Utility Functions ======
const badge = (v) =>
  v === 'internal' ? `<span class="pill int">Internal 🔒</span>` : `<span class="pill ext">Customer ✅</span>`;
...
const highlight = (t, q) => (!q ? t : t.replace(new RegExp(`(${q})`, 'gi'), '<mark>$1</mark>'));

const filtered = (items, q) =>
  !q ? items : items.filter((i) => i.title.toLowerCase().includes(q) || (i.notes || '').toLowerCase().includes(q));

// ====== Render ======
function render() {
  document.getElementById('sideNav').innerHTML = DATA.groups.map((g) => `<a href="#${g.id}">${g.title}</a>`).join('');

  const q = (qEl.value || '').trim().toLowerCase();
  const all = DATA.groups.flatMap((g) => g.items);
  const favItems = all.filter((i) => FAVS.includes(i.title));
  const groups = [{ id: 'favs', title: '⭐ Favourites', items: favItems }, ...DATA.groups];
...
themeBtn.onclick = () => {
  ...
  localStorage.setItem('agent.theme', document.body.classList.contains('dark') ? 'dark' : 'light');
};
if (localStorage.getItem('agent.theme') === 'dark') document.body.classList.add('dark');
