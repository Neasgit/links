// dashboard.js
// Theme / button consistency fixes + responsive color select sync
  ...
  function initDashboard(data) {
    ...
    if (paletteEl) {
      ...
      COLORS.forEach(c => {
        ...
        btn.addEventListener('click', () => {
          ...
          if (metaTheme) metaTheme.setAttribute('content', c.v);
    ...
    if (colorSelect) {
      ...
      colorSelect.addEventListener('change', () => {
        ...
        if (metaTheme) metaTheme.setAttribute('content', v);
        if (paletteEl) paletteEl.querySelectorAll('.color-dot').forEach(d => d.classList.remove('active'));
        const activeBtn = paletteEl && Array.from(paletteEl.children).find(b => b.style.background === v);
        if (activeBtn) activeBtn.classList.add('active');
      });
    }

    // Theme toggle: update both html & body datasets, and meta theme color
    attach(themeBtn, 'click', (e) => {
      ...
      const accent = localStorage.getItem('accent') || getComputedStyle(document.documentElement).getPropertyValue('--accent').trim();
    ...
    // Grid size cycle (keeps layout responsive)
    ...
    attach(gridSizeBtn, 'click', (e) => {
      ...
      idx = (idx + 1) % (GRID_KEYS.length + 1); // +1 = auto
      if (idx < GRID_KEYS.length) {
        document.body.classList.add(GRID_KEYS[idx]);
        setLS('grid-class', GRID_KEYS[idx]);
      } else rmLS('grid-class');
      // button label shows columns when class present
      if (gridSizeBtn) gridSizeBtn.textContent = (idx < GRID_KEYS.length) ? ((idx+1) + '▦') : '▦';
    ...
    // keep icon texts in sync if classes changed elsewhere
    ...
    // card creation: header row with star so no overlap
    function createCard(item){
      const card = document.createElement('div'); card.className='card';
      const header = document.createElement('div'); header.className='card-header';
      const strong = document.createElement('strong'); strong.textContent = item.title || 'Untitled';
      header.appendChild(strong);
      const starBtn = document.createElement('button'); starBtn.type='button'; starBtn.className='star';
